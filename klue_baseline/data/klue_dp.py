...
class KlueDPProcessor(DataProcessor):
    ...
    origin_train_file_name = "result-head_final.txt"# 20211008
    ...
    def convert_examples_to_features(
        ...
        for example in examples:
            if SENT_ID != example.sent_id:
                ...
                for token, head, dep, pos in zip(token_list, head_list, dep_list, pos_list):
                    ...
                    dep_mask = [-1 if dep not in get_dep_labels() else dep_label_map[dep]] + [-1] * (bpe_len - 1)
                    dep_ids.extend(dep_mask)
                    # pos_mask = [pos_label_map[pos]] + [-1] * (bpe_len - 1)
                    pos_mask = [pos_label_map['NA'] if pos not in get_pos_labels() else pos_label_map[pos]] + [-1] * (bpe_len - 1)
        ...
        for token, head, dep, pos in zip(token_list, head_list, dep_list, pos_list):
            ...
            dep_ids.extend(dep_mask)
            # pos_mask = [pos_label_map[pos]] + [-1] * (bpe_len - 1)
            pos_mask = [ -1 if pos not in get_pos_labels() else pos_label_map[pos]] + [-1] * (bpe_len - 1)
            pos_ids.extend(pos_mask)
